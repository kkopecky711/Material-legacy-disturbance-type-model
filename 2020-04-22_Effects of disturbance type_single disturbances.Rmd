...
To do list:
  ...
  - Figure out how to make time to reach pre-disturbance cover figure rather than time to equilibrium
  - Integrate density dependence into herbivory and recruitment terms in ODE's
  - Figure out three-layer for loop() for reduced herbivory sensitivity analysis
  - Sensitivity analyses of recruitment and erosion rates?
...
tset.disturb <- seq(from = 0, to = 6000) # Time vector for inner loop
```
...
tset_phys_dist <- seq(from = 0, to = 6000)
...
legend(x=450,y=0.7,legend=c('Live coral','Dead coral','Macroalgae (open substrate)','Macroalgae (dead coral)'),lwd=1,lty='solid',col=c('blue','black','green','grey'))
abline(v=1000, lty = 'dashed')
#abline(v=220, lty = 'dashed')
#abline(v=340, lty = 'dashed')
#abline(v=460, lty = 'dashed')

#which(round(C_L.simu_phys[(t_disturb+1):length(tset.disturb)], 2) == round(C_L.simu_phys[499], 2))[1]
```
...
# Create time vector, holding vectors for each state variable, and set initial conditions
tset_bio_dist <- seq(from = 0, to = 6000)
...
C_D.simu_phys <- NaN*tset_phys_dist; C_D.simu_phys[1] <- 0.19
...
C_D.bio <- NaN*tset_bio_dist; C_D.bio[1] <- 0.19
...
# create holding vectors for equilibrium cover following disturbance for all space holders
...
for(j in 1:length(disturb_set)){
  ...
  # create holding vectors for the inner loop
	...
	C_D.phys.single.algae_state[1] <- 0.19
...
for(j in 1:length(disturb_set)){
  ...
  # create holding vectors for the inner loop
	...
	C_D.bio.single.algae_state[1] <- 0.19
...
lines(disturb_set, c(M_C.phys.eq_cover.single.coral_state + M_S.phys.eq_cover.single.coral_state), col = "green", lwd = 2, lty = 'solid', type = 'l')
lines(disturb_set, c(M_C.bio.eq_cover.single.coral_state + M_S.bio.eq_cover.single.coral_state), col = "green", lwd = 2, lty = 'dashed', type = 'l')
legend(x=0.6,y=1.1,legend=c('Live coral','Total macroalgae', 'Structure-removing', 'Structure-retaining'), lwd=1,lty=c('solid','solid','solid','dashed'),col=c('blue','green', 'black','black'))
...
lines(disturb_set, C_L.bio.eq_cover.single.algae_state, col = "blue", lwd = 2, lty = 'dashed', type = 'l')
lines(disturb_set, c(M_C.phys.eq_cover.single.algae_state + M_S.phys.eq_cover.single.algae_state), col = "green", lwd = 2, lty = 'solid', type = 'l')
lines(disturb_set, c(M_C.bio.eq_cover.single.algae_state + M_S.bio.eq_cover.single.algae_state), col = "green", lwd = 2, lty = 'dashed', type = 'l')

#hist(C_L.bio.eq_cover.single.coral_state, breaks = 100)
#hist(c(M_C.bio.eq_cover.single.algae_state + M_S.bio.eq_cover.single.algae_state), breaks = 100)

plot(disturb_set, C_D.phys.eq_cover.single.coral_state, col = "black", lwd = 2, lty = 'solid', type = 'l', xlab = "Severity of disturbance", ylab = "Equilibrium cover", xlim = c(0, 1), ylim = c(0, 1.1), main = "Dead coral by disturbance type, coral state")
lines(disturb_set, C_D.bio.eq_cover.single.coral_state, col = "black", lwd = 2, lty = 'dashed', type = 'l')
...
# Heat map attempts
#disturb_set <- as.numeric(disturb_set)
#C_L.bio.eq_cover.single.coral_state

#library(tidyverse)
#library(dplyr)

#live_coral_heatmap <- cbind(disturb_set, C_L.bio.eq_cover.single.coral_state)

#heatmap(live_coral_heatmap)
```
...
#t_disturb.t_eq <- 2000
disturb_set.t_eq <- seq(from = 0, to = 0.31, by = .01)#.01) # Time vector for outer loop
tset.disturb <- seq(from = 0, to = 8000)#1500) # Time vector for inner loop
...
# for each value of 'disturb_set'
  for(i in 2:length(tset.disturb)){
    ...
    dM_C <- (dt/5)*(g_M*M_C*C_D + g_M*M_S*C_D + g_M_C_L*C_L*(M_S + M_C) + gamma_M*C_D - M_C*(h_M_C + d_M + e))
  	...
  	#pre_disturb_C_L[j] <- which(round(C_L.bio[(t_disturb+1):length(tset.disturb)], 2) == round(C_L.bio[499], 2))
    }
  }
...
#pre_disturb_C_L[j] <- which(round(C_L.bio[(t_disturb+1):length(tset.disturb)], 2) == round(C_L.bio[499], 2))[1]

#time_pre_disturb_level.C_L[j] <- which(tset.disturb(t_disturb+1):length(tset.disturb))pre_disturb_level.C_L[1]
}

```

```{r Time to equilibrium phys dist, echo=FALSE}
...
plot(disturb_set.t_eq,system_equilibration.bio, col = "black", lwd = 2, lty = 'dashed', type = 'l', xlab = "Severity of disturbance", ylab = "Time to equilibrium (months)", ylim = c(0,5000))
lines(disturb_set.t_eq,system_equilibration.phys,col = "black", lwd = 2, lty = 'solid', type = 'l')
legend(x=.2,y=1100,legend=c('Structure-retaining', 'Structure-removing'),lwd=1,lty=c('dashed', 'solid'),col=c('black', 'black'))
...
for(i in 2:length(tset_phys_dist.ten_year)){ # For every element after the first in the time vector...
  ...
  # reduce all space holders to 50% of their pre-disturbance levels
  C_L <- C_L.simu_phys.ten_year[i-1]*0.5
  C_D <- C_D.simu_phys.ten_year[i-1]*0.5
  M_S <- M_S.simu_phys.ten_year[i-1]*0.5
  M_C <- M_C.simu_phys.ten_year[i-1]*0.5
...
```{r Reduced herbivory sensitivity analysis, echo=FALSE}
## Structure-removing disturbances
h_M_C.set <- seq(from = 0, to = 0.3, by = 0.01)

# Holding vectors for outermost loop
C_L.red_herb <- NaN*h_M_C.set
C_D.red_herb <- NaN*h_M_C.set
M_S.red_herb <- NaN*h_M_C.set
M_C.red_herb <- NaN*h_M_C.set

# Outermost loop
for(k in 1:length(h_M_C.set)){

# Holding vectors for equilibration time following disturbance for all space holders (middle loop)
C_L.phys.eq_cover.single.coral_state <- NaN*disturb_set
C_D.phys.eq_cover.single.coral_state <- NaN*disturb_set
M_S.phys.eq_cover.single.coral_state <- NaN*disturb_set
M_C.phys.eq_cover.single.coral_state <- NaN*disturb_set
  ...
  for(i in 2:length(tset.disturb)){
   ...
   if(tset.disturb[i] !=t_disturb){ # change in time
    ...
    M_S <- M_S.phys.single.coral_state[i-1]
  	...
  	# calculate change in proportional cover at each timestep
    dC_L <- (dt/5)*(g_C*C_L*(1-C_L-C_D-M_S-M_C) + gamma_C*(1-C_L-C_D-M_S-M_C) - g_M_C_L*C_L*(M_S + M_C) - d_C*C_L) 
    dC_D <- (dt/5)*(d_C*C_L + M_C*(h_M_C.set + d_M) - g_M*C_D*(M_S + M_C) - e*C_D)
    dM_S <- (dt/5)*(g_M*M_S*(1-C_L-C_D-M_S-M_C) + g_M*M_C*(1-C_L-C_D-M_S-M_C) + gamma_M*(1-C_L-C_D-M_S-M_C) - M_S*(h_M_S + d_M))
    dM_C <- (dt/5)*(g_M*M_C*C_D + g_M*M_S*C_D + g_M_C_L*C_L*(M_S + M_C) + gamma_M*C_D - M_C*(h_M_C.set + d_M + e))
...
C_L.red_herb[k] <- C_L.phys.eq_cover.single.coral_state[k]
C_D.red_herb[k] <- C_D.phys.eq_cover.single.coral_state[k]
M_S.red_herb[k] <- M_S.phys.eq_cover.single.coral_state[k]
M_C.red_herb[k] <- M_C.phys.eq_cover.single.coral_state[k]
}
...
```{r Erosion rate bifurcation diagram, echo = FALSE}

## Coral-dominated state
# Create time vector, holding vectors for each state variable, and set initial conditions
tset.e_rate <- seq(from = 0, to = 20000)
e_set <- seq(from = 0, to = 0.02, by = 0.001)

C_L.eq_cover.e_rate.coral_state <- NaN*e_set
C_D.eq_cover.e_rate.coral_state <- NaN*e_set
M_S.eq_cover.e_rate.coral_state <- NaN*e_set
M_C.eq_cover.e_rate.coral_state <- NaN*e_set

for(j in 1:length(e_set)){
  
  e_rate <- e_set[j]
  
  C_L.simu.e_rate.coral_state <- NaN*tset.e_rate; C_L.simu.e_rate.coral_state[1] <- 0.6
  C_D.simu.e_rate.coral_state <- NaN*tset.e_rate; C_D.simu.e_rate.coral_state[1] <- 0
  M_S.simu.e_rate.coral_state <- NaN*tset.e_rate; M_S.simu.e_rate.coral_state[1] <- 0.05
  M_C.simu.e_rate.coral_state <- NaN*tset.e_rate; M_C.simu.e_rate.coral_state[1] <- 0.05
  
  
  for(i in 2:length(tset.e_rate)){ # For every element after the first in the time vector...
    
    dt <- tset.e_rate[i] - tset.e_rate[i-1] # calculate change in time  
   
    # Proportional cover at previous time step  
    C_L <- C_L.simu.e_rate.coral_state[i-1]
    C_D <- C_D.simu.e_rate.coral_state[i-1]
    M_S <- M_S.simu.e_rate.coral_state[i-1]
    M_C <- M_C.simu.e_rate.coral_state[i-1]
    
    # Change in proportional cover, converted to units of months (/5)
    dC_L <- (dt/5)*(g_C*C_L*(1-C_L-C_D-M_S-M_C) + gamma_C*(1-C_L-C_D-M_S-M_C) - g_M_C_L*C_L*(M_S + M_C) - d_C*C_L) 
    dC_D <- (dt/5)*(d_C*C_L + M_C*(h_M_C + d_M) - g_M*C_D*(M_S + M_C) - e_rate*C_D)
    dM_S <- (dt/5)*(g_M*M_S*(1-C_L-C_D-M_S-M_C) + g_M*M_C*(1-C_L-C_D-M_S-M_C) + gamma_M*(1-C_L-C_D-M_S-M_C) - M_S*(h_M_S + d_M))
    dM_C <- (dt/5)*(g_M*M_C*C_D + g_M*M_S*C_D + g_M_C_L*C_L*(M_S + M_C) + gamma_M*C_D - M_C*(h_M_C + d_M + e_rate))
   
    # Proportional cover at new time step   
    C_L.simu.e_rate.coral_state[i] <- C_L.simu.e_rate.coral_state[i-1] + dC_L
    C_D.simu.e_rate.coral_state[i] <- C_D.simu.e_rate.coral_state[i-1] + dC_D
    M_S.simu.e_rate.coral_state[i] <- M_S.simu.e_rate.coral_state[i-1] + dM_S
    M_C.simu.e_rate.coral_state[i] <- M_C.simu.e_rate.coral_state[i-1] + dM_C
  }
  
  C_L.eq_cover.e_rate.coral_state[j] <- C_L.simu.e_rate.coral_state[max(tset.e_rate)]
  C_D.eq_cover.e_rate.coral_state[j] <- C_D.simu.e_rate.coral_state[max(tset.e_rate)]
  M_S.eq_cover.e_rate.coral_state[j] <- M_S.simu.e_rate.coral_state[max(tset.e_rate)]
  M_C.eq_cover.e_rate.coral_state[j] <- M_C.simu.e_rate.coral_state[max(tset.e_rate)]
}
...
C_L.eq_cover.e_rate.algal_state <- NaN*e_set
C_D.eq_cover.e_rate.algal_state <- NaN*e_set
M_S.eq_cover.e_rate.algal_state <- NaN*e_set
M_C.eq_cover.e_rate.algal_state <- NaN*e_set

for(j in 1:length(e_set)){
  
  e_rate <- e_set[j]
  
  C_L.simu.e_rate.algal_state <- NaN*tset.e_rate; C_L.simu.e_rate.algal_state[1] <- 0.1 #0.01
  C_D.simu.e_rate.algal_state <- NaN*tset.e_rate; C_D.simu.e_rate.algal_state[1] <- 0.19 #0.23
  M_S.simu.e_rate.algal_state <- NaN*tset.e_rate; M_S.simu.e_rate.algal_state[1] <- 0.18 #0.19
  M_C.simu.e_rate.algal_state <- NaN*tset.e_rate; M_C.simu.e_rate.algal_state[1] <- 0.28 #0.39
  ...
  for(i in 2:length(tset.e_rate)){ # For every element after the first in the time vector...
    ...
    # Proportional cover at previous time step  
    C_L <- C_L.simu.e_rate.algal_state[i-1]
    C_D <- C_D.simu.e_rate.algal_state[i-1]
    M_S <- M_S.simu.e_rate.algal_state[i-1]
    M_C <- M_C.simu.e_rate.algal_state[i-1]
    ...
    # Proportional cover at new time step   
    C_L.simu.e_rate.algal_state[i] <- C_L.simu.e_rate.algal_state[i-1] + dC_L
    C_D.simu.e_rate.algal_state[i] <- C_D.simu.e_rate.algal_state[i-1] + dC_D
    M_S.simu.e_rate.algal_state[i] <- M_S.simu.e_rate.algal_state[i-1] + dM_S
    M_C.simu.e_rate.algal_state[i] <- M_C.simu.e_rate.algal_state[i-1] + dM_C
  }
  
  C_L.eq_cover.e_rate.algal_state[j] <- C_L.simu.e_rate.algal_state[max(tset.e_rate)]
  C_D.eq_cover.e_rate.algal_state[j] <- C_D.simu.e_rate.algal_state[max(tset.e_rate)]
  M_S.eq_cover.e_rate.algal_state[j] <- M_S.simu.e_rate.algal_state[max(tset.e_rate)]
  M_C.eq_cover.e_rate.algal_state[j] <- M_C.simu.e_rate.algal_state[max(tset.e_rate)]
}


## Plot bifurcation diagrams
# Coral-dominated state, all space holders
#plot(e_set, C_L.eq_cover.e_rate.coral_state, col = "blue", lwd = 2, lty = 'solid', type = 'l', xlab = "Erosion rate", ylab = "Equilibrium cover", xlim = c(0, max(e_set)), ylim = c(0, 1), main = "Coral-dominated state")
#lines(e_set, (M_S.eq_cover.e_rate.coral_state + M_C.eq_cover.e_rate.coral_state), col = "green", lwd = 2, lty = 'solid', type = 'l')
#lines(e_set, C_D.eq_cover.e_rate.coral_state, col = "black", lwd = 2, lty = 'solid', type = 'l')
#legend(x=0.01, y=0.6,legend=c('Live coral','Dead coral', 'Total macroalgae'), col = c("blue", "black", "green"), lty = c('solid', 'solid'))

# Macroalgae-dominated state, all space holders
#plot(e_set, C_L.eq_cover.e_rate.algal_state, col = "blue", lwd = 2, lty = 'solid', type = 'l', xlab = "Erosion rate", ylab = "Equilibrium cover", xlim = c(0, max(e_set)), ylim = c(0, 1), main = "Macroalgae-dominated state")
#lines(e_set, (M_S.eq_cover.e_rate.algal_state + M_C.eq_cover.e_rate.algal_state), col = "green", lwd = 2, lty = 'solid', type = 'l')
#lines(e_set, C_D.eq_cover.e_rate.algal_state, col = "black", lwd = 2, lty = 'solid', type = 'l')
#legend(x=0.01, y=0.6,legend=c('Live coral','Dead coral', 'Total macroalgae'), col = c("blue", "black", "green"), lty = c('solid', 'solid'))

# Live coral, both states
plot(e_set, C_L.eq_cover.e_rate.coral_state, col = "blue", lwd = 2, lty = 'solid', type = 'l', xlab = "Erosion rate", ylab = "Equilibrium coral cover", xlim = c(0, max(e_set)), ylim = c(0, 1), main = "Live coral")
lines(e_set, C_L.eq_cover.e_rate.algal_state, col = "blue", lwd = 2, lty = 'dashed', type = 'l')
legend(x=-0.0005, y=1,legend=c('Coral-dominated','Algae-dominated'), col = "blue", lty = c('solid', 'dashed'), title = "Initial community state", box.lty=0)

# Total macroalgae, both states
plot(e_set, (M_S.eq_cover.e_rate.coral_state + M_C.eq_cover.e_rate.coral_state), col = "green", lwd = 2, lty = 'solid', type = 'l', xlab = "Erosion rate", ylab = "Equilibrium algae cover", xlim = c(0, max(e_set)), ylim = c(0, 1), main = "Total macroalgae")
lines(e_set, (M_S.eq_cover.e_rate.algal_state + M_C.eq_cover.e_rate.algal_state), col = "green", lwd = 2, lty = 'dashed', type = 'l')
legend(x=0.0135, y=1,legend=c('Coral-dominated','Algae-dominated'), col = "green", lty = c('solid', 'dashed'), title = "Initial community state", box.lty=0)
```

```{r Herbivory rate bifurcation diagram, echo = FALSE}

## Initially coral-dominated state
# Create time vector, holding vectors for each state variable, and set initial conditions
tset.herb_rate <- seq(from = 0, to = 20000)
herb_set <- seq(from = 0, to = 0.3, by = 0.01)

C_L.eq_cover.herb_rate.coral_state <- NaN*herb_set
C_D.eq_cover.herb_rate.coral_state <- NaN*herb_set
M_S.eq_cover.herb_rate.coral_state <- NaN*herb_set
M_C.eq_cover.herb_rate.coral_state <- NaN*herb_set

for(j in 1:length(herb_set)){
  
  herb_rate <- herb_set[j]
  
  C_L.simu.herb_rate.coral_state <- NaN*tset.herb_rate; C_L.simu.herb_rate.coral_state[1] <- 0.6
  C_D.simu.herb_rate.coral_state <- NaN*tset.herb_rate; C_D.simu.herb_rate.coral_state[1] <- 0
  M_S.simu.herb_rate.coral_state <- NaN*tset.herb_rate; M_S.simu.herb_rate.coral_state[1] <- 0.05
  M_C.simu.herb_rate.coral_state <- NaN*tset.herb_rate; M_C.simu.herb_rate.coral_state[1] <- 0.05
  
  
  for(i in 2:length(tset.herb_rate)){ # For every element after the first in the time vector...
    
    dt <- tset.herb_rate[i] - tset.herb_rate[i-1] # calculate change in time  
   
    # Proportional cover at previous time step  
    C_L <- C_L.simu.herb_rate.coral_state[i-1]
    C_D <- C_D.simu.herb_rate.coral_state[i-1]
    M_S <- M_S.simu.herb_rate.coral_state[i-1]
    M_C <- M_C.simu.herb_rate.coral_state[i-1]
    
    # Change in proportional cover, converted to units of months (/5)
    dC_L <- (dt/5)*(g_C*C_L*(1-C_L-C_D-M_S-M_C) + gamma_C*(1-C_L-C_D-M_S-M_C) - g_M_C_L*C_L*(M_S + M_C) - d_C*C_L) 
    dC_D <- (dt/5)*(d_C*C_L + M_C*(herb_rate + d_M) - g_M*C_D*(M_S + M_C) - e*C_D)
    dM_S <- (dt/5)*(g_M*M_S*(1-C_L-C_D-M_S-M_C) + g_M*M_C*(1-C_L-C_D-M_S-M_C) + gamma_M*(1-C_L-C_D-M_S-M_C) - M_S*(h_M_S + d_M))
    dM_C <- (dt/5)*(g_M*M_C*C_D + g_M*M_S*C_D + g_M_C_L*C_L*(M_S + M_C) + gamma_M*C_D - M_C*(herb_rate + d_M + e))
   
    # Proportional cover at new time step   
    C_L.simu.herb_rate.coral_state[i] <- C_L.simu.herb_rate.coral_state[i-1] + dC_L
    C_D.simu.herb_rate.coral_state[i] <- C_D.simu.herb_rate.coral_state[i-1] + dC_D
    M_S.simu.herb_rate.coral_state[i] <- M_S.simu.herb_rate.coral_state[i-1] + dM_S
    M_C.simu.herb_rate.coral_state[i] <- M_C.simu.herb_rate.coral_state[i-1] + dM_C
  }
  
  C_L.eq_cover.herb_rate.coral_state[j] <- C_L.simu.herb_rate.coral_state[max(tset.herb_rate)]
  C_D.eq_cover.herb_rate.coral_state[j] <- C_D.simu.herb_rate.coral_state[max(tset.herb_rate)]
  M_S.eq_cover.herb_rate.coral_state[j] <- M_S.simu.herb_rate.coral_state[max(tset.herb_rate)]
  M_C.eq_cover.herb_rate.coral_state[j] <- M_C.simu.herb_rate.coral_state[max(tset.herb_rate)]
}

## Initially macroalgae-dominated community state
# Create time vector, holding vectors for each state variable, and set initial conditions
tset.herb_rate <- seq(from = 0, to = 20000)
herb_set <- seq(from = 0, to = 0.3, by = 0.01)

C_L.eq_cover.herb_rate.algal_state <- NaN*herb_set
C_D.eq_cover.herb_rate.algal_state <- NaN*herb_set
M_S.eq_cover.herb_rate.algal_state <- NaN*herb_set
M_C.eq_cover.herb_rate.algal_state <- NaN*herb_set

for(j in 1:length(herb_set)){
  
  herb_rate <- herb_set[j]
  
  C_L.simu.herb_rate.algal_state <- NaN*tset.herb_rate; C_L.simu.herb_rate.algal_state[1] <- 0.1
  C_D.simu.herb_rate.algal_state <- NaN*tset.herb_rate; C_D.simu.herb_rate.algal_state[1] <- 0.19
  M_S.simu.herb_rate.algal_state <- NaN*tset.herb_rate; M_S.simu.herb_rate.algal_state[1] <- 0.18
  M_C.simu.herb_rate.algal_state <- NaN*tset.herb_rate; M_C.simu.herb_rate.algal_state[1] <- 0.28
  ...
  for(i in 2:length(tset.herb_rate)){ # For every element after the first in the time vector...
    ...
    # Proportional cover at previous time step  
    C_L <- C_L.simu.herb_rate.algal_state[i-1]
    C_D <- C_D.simu.herb_rate.algal_state[i-1]
    M_S <- M_S.simu.herb_rate.algal_state[i-1]
    M_C <- M_C.simu.herb_rate.algal_state[i-1]
    ...
    # Proportional cover at new time step   
    C_L.simu.herb_rate.algal_state[i] <- C_L.simu.herb_rate.algal_state[i-1] + dC_L
    C_D.simu.herb_rate.algal_state[i] <- C_D.simu.herb_rate.algal_state[i-1] + dC_D
    M_S.simu.herb_rate.algal_state[i] <- M_S.simu.herb_rate.algal_state[i-1] + dM_S
    M_C.simu.herb_rate.algal_state[i] <- M_C.simu.herb_rate.algal_state[i-1] + dM_C
  }
  
  C_L.eq_cover.herb_rate.algal_state[j] <- C_L.simu.herb_rate.algal_state[max(tset.herb_rate)]
  C_D.eq_cover.herb_rate.algal_state[j] <- C_D.simu.herb_rate.algal_state[max(tset.herb_rate)]
  M_S.eq_cover.herb_rate.algal_state[j] <- M_S.simu.herb_rate.algal_state[max(tset.herb_rate)]
  M_C.eq_cover.herb_rate.algal_state[j] <- M_C.simu.herb_rate.algal_state[max(tset.herb_rate)]
}


## Bifrucation diagrams
# Live coral, both states
plot(herb_set, C_L.eq_cover.herb_rate.coral_state, col = "blue", lwd = 2, lty = 'solid', type = 'l', xlab = "Herbivory rate (h_m_c)", ylab = "Equilibrium coral cover", xlim = c(0, max(herb_set)), ylim = c(0, 1), main = "Live coral")
lines(herb_set, C_L.eq_cover.herb_rate.algal_state, col = "blue", lwd = 2, lty = 'dashed', type = 'l')
legend(x=-0.01, y=1,legend=c('Coral-dominated','Algae-dominated'), col = "blue", lty = c('solid', 'dashed'), title = "Initial community state", box.lty=0)

# Total macroalgae, both states
plot(herb_set, (M_S.eq_cover.herb_rate.coral_state + M_C.eq_cover.herb_rate.coral_state), col = "green", lwd = 2, lty = 'solid', type = 'l', xlab = "Herbivory rate (h_m_c)", ylab = "Equilibrium algae cover", xlim = c(0, max(herb_set)), ylim = c(0, 1), main = "Total macroalgae")
lines(herb_set, (M_S.eq_cover.herb_rate.algal_state + M_C.eq_cover.herb_rate.algal_state), col = "green", lwd = 2, lty = 'dashed', type = 'l')
legend(x=0.0135, y=1,legend=c('Coral-dominated','Algae-dominated'), col = "green", lty = c('solid', 'dashed'), title = "Initial community state", box.lty=0)


```

```{r Coral recruitment rate bifurcation diagram, echo = FALSE}

## Initially coral-dominated state
# Create time vector, holding vectors for each state variable, and set initial conditions
tset.coral_recruit_rate <- seq(from = 0, to = 20000)
recruit_set <- seq(from = 0, to = 0.1, by = 0.001)

C_L.eq_cover.coral_recruit_rate.coral_state <- NaN*recruit_set
C_D.eq_cover.coral_recruit_rate.coral_state <- NaN*recruit_set
M_S.eq_cover.coral_recruit_rate.coral_state <- NaN*recruit_set
M_C.eq_cover.coral_recruit_rate.coral_state <- NaN*recruit_set

for(j in 1:length(recruit_set)){
  
  recruit_rate <- recruit_set[j]
  
  C_L.simu.coral_recruit_rate.coral_state <- NaN*tset.coral_recruit_rate; C_L.simu.coral_recruit_rate.coral_state[1] <- 0.6
  C_D.simu.coral_recruit_rate.coral_state <- NaN*tset.coral_recruit_rate; C_D.simu.coral_recruit_rate.coral_state[1] <- 0
  M_S.simu.coral_recruit_rate.coral_state <- NaN*tset.coral_recruit_rate; M_S.simu.coral_recruit_rate.coral_state[1] <- 0.05
  M_C.simu.coral_recruit_rate.coral_state <- NaN*tset.coral_recruit_rate; M_C.simu.coral_recruit_rate.coral_state[1] <- 0.05
  
  
  for(i in 2:length(tset.coral_recruit_rate)){ # For every element after the first in the time vector...
    
    dt <- tset.coral_recruit_rate[i] - tset.coral_recruit_rate[i-1] # calculate change in time  
   
    # Proportional cover at previous time step  
    C_L <- C_L.simu.coral_recruit_rate.coral_state[i-1]
    C_D <- C_D.simu.coral_recruit_rate.coral_state[i-1]
    M_S <- M_S.simu.coral_recruit_rate.coral_state[i-1]
    M_C <- M_C.simu.coral_recruit_rate.coral_state[i-1]
    
    # Change in proportional cover, converted to units of months (/5)
    dC_L <- (dt/5)*(g_C*C_L*(1-C_L-C_D-M_S-M_C) + recruit_rate*(1-C_L-C_D-M_S-M_C) - g_M_C_L*C_L*(M_S + M_C) - d_C*C_L) 
    dC_D <- (dt/5)*(d_C*C_L + M_C*(h_M_C + d_M) - g_M*C_D*(M_S + M_C) - e*C_D)
    dM_S <- (dt/5)*(g_M*M_S*(1-C_L-C_D-M_S-M_C) + g_M*M_C*(1-C_L-C_D-M_S-M_C) + gamma_M*(1-C_L-C_D-M_S-M_C) - M_S*(h_M_S + d_M))
    dM_C <- (dt/5)*(g_M*M_C*C_D + g_M*M_S*C_D + g_M_C_L*C_L*(M_S + M_C) + gamma_M*C_D - M_C*(h_M_C + d_M + e))
   
    # Proportional cover at new time step   
    C_L.simu.coral_recruit_rate.coral_state[i] <- C_L.simu.coral_recruit_rate.coral_state[i-1] + dC_L
    C_D.simu.coral_recruit_rate.coral_state[i] <- C_D.simu.coral_recruit_rate.coral_state[i-1] + dC_D
    M_S.simu.coral_recruit_rate.coral_state[i] <- M_S.simu.coral_recruit_rate.coral_state[i-1] + dM_S
    M_C.simu.coral_recruit_rate.coral_state[i] <- M_C.simu.coral_recruit_rate.coral_state[i-1] + dM_C
  }
  
  C_L.eq_cover.coral_recruit_rate.coral_state[j] <- C_L.simu.coral_recruit_rate.coral_state[max(tset.coral_recruit_rate)]
  C_D.eq_cover.coral_recruit_rate.coral_state[j] <- C_D.simu.coral_recruit_rate.coral_state[max(tset.coral_recruit_rate)]
  M_S.eq_cover.coral_recruit_rate.coral_state[j] <- M_S.simu.coral_recruit_rate.coral_state[max(tset.coral_recruit_rate)]
  M_C.eq_cover.coral_recruit_rate.coral_state[j] <- M_C.simu.coral_recruit_rate.coral_state[max(tset.coral_recruit_rate)]
}

## Initially macroalgae-dominated community state
# Create time vector, holding vectors for each state variable, and set initial conditions

C_L.eq_cover.coral_recruit_rate.algal_state <- NaN*recruit_set
C_D.eq_cover.coral_recruit_rate.algal_state <- NaN*recruit_set
M_S.eq_cover.coral_recruit_rate.algal_state <- NaN*recruit_set
M_C.eq_cover.coral_recruit_rate.algal_state <- NaN*recruit_set

for(j in 1:length(recruit_set)){
  
  recruit_rate <- recruit_set[j]
  
  C_L.simu.coral_recruit_rate.algal_state <- NaN*tset.coral_recruit_rate; C_L.simu.coral_recruit_rate.algal_state[1] <- 0.1
  C_D.simu.coral_recruit_rate.algal_state <- NaN*tset.coral_recruit_rate; C_D.simu.coral_recruit_rate.algal_state[1] <- 0.19
  M_S.simu.coral_recruit_rate.algal_state <- NaN*tset.coral_recruit_rate; M_S.simu.coral_recruit_rate.algal_state[1] <- 0.18
  M_C.simu.coral_recruit_rate.algal_state <- NaN*tset.coral_recruit_rate; M_C.simu.coral_recruit_rate.algal_state[1] <- 0.28
  ...
  for(i in 2:length(tset.coral_recruit_rate)){ # For every element after the first in the time vector...
    ...
    # Proportional cover at previous time step  
    C_L <- C_L.simu.coral_recruit_rate.algal_state[i-1]
    C_D <- C_D.simu.coral_recruit_rate.algal_state[i-1]
    M_S <- M_S.simu.coral_recruit_rate.algal_state[i-1]
    M_C <- M_C.simu.coral_recruit_rate.algal_state[i-1]
    ...
    # Proportional cover at new time step   
    C_L.simu.coral_recruit_rate.algal_state[i] <- C_L.simu.coral_recruit_rate.algal_state[i-1] + dC_L
    C_D.simu.coral_recruit_rate.algal_state[i] <- C_D.simu.coral_recruit_rate.algal_state[i-1] + dC_D
    M_S.simu.coral_recruit_rate.algal_state[i] <- M_S.simu.coral_recruit_rate.algal_state[i-1] + dM_S
    M_C.simu.coral_recruit_rate.algal_state[i] <- M_C.simu.coral_recruit_rate.algal_state[i-1] + dM_C
  }
  
  C_L.eq_cover.coral_recruit_rate.algal_state[j] <- C_L.simu.coral_recruit_rate.algal_state[max(tset.coral_recruit_rate)]
  C_D.eq_cover.coral_recruit_rate.algal_state[j] <- C_D.simu.coral_recruit_rate.algal_state[max(tset.coral_recruit_rate)]
  M_S.eq_cover.coral_recruit_rate.algal_state[j] <- M_S.simu.coral_recruit_rate.algal_state[max(tset.coral_recruit_rate)]
  M_C.eq_cover.coral_recruit_rate.algal_state[j] <- M_C.simu.coral_recruit_rate.algal_state[max(tset.coral_recruit_rate)]
}


## Bifrucation diagrams
# Live coral, both states
plot(recruit_set, C_L.eq_cover.coral_recruit_rate.coral_state, col = "blue", lwd = 2, lty = 'solid', type = 'l', xlab = "Coral recruitment rate", ylab = "Equilibrium coral cover", xlim = c(0, max(recruit_set)), ylim = c(0, 1), main = "Live coral")
lines(recruit_set, C_L.eq_cover.coral_recruit_rate.algal_state, col = "blue", lwd = 2, lty = 'dashed', type = 'l')
legend(x=-0.01, y=1,legend=c('Coral-dominated','Algae-dominated'), col = "blue", lty = c('solid', 'dashed'), title = "Initial community state", box.lty=0)

# Total macroalgae, both states
plot(recruit_set, (M_S.eq_cover.coral_recruit_rate.coral_state + M_C.eq_cover.coral_recruit_rate.coral_state), col = "green", lwd = 2, lty = 'solid', type = 'l', xlab = "Coral recruitment rate", ylab = "Equilibrium algae cover", xlim = c(0, max(recruit_set)), ylim = c(0, 1), main = "Total macroalgae")
lines(recruit_set, (M_S.eq_cover.coral_recruit_rate.algal_state + M_C.eq_cover.coral_recruit_rate.algal_state), col = "green", lwd = 2, lty = 'dashed', type = 'l')
legend(x=0.0135, y=1,legend=c('Coral-dominated','Algae-dominated'), col = "green", lty = c('solid', 'dashed'), title = "Initial community state", box.lty=0)
