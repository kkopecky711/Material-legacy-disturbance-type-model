...
for(j in 1:length(e_set)){
  ...
  C_L.simu.e_rate.algal_state <- NaN*tset.e_rate; C_L.simu.e_rate.algal_state[1] <- 0.01
  C_D.simu.e_rate.algal_state <- NaN*tset.e_rate; C_D.simu.e_rate.algal_state[1] <- 0.23
  M_S.simu.e_rate.algal_state <- NaN*tset.e_rate; M_S.simu.e_rate.algal_state[1] <- 0.19
  M_C.simu.e_rate.algal_state <- NaN*tset.e_rate; M_C.simu.e_rate.algal_state[1] <- 0.39
...
C_L.eq_cover.herb_rate <- NaN*herb_set
C_D.eq_cover.herb_rate <- NaN*herb_set
M_S.eq_cover.herb_rate <- NaN*herb_set
M_C.eq_cover.herb_rate <- NaN*herb_set

for(j in 1:length(herb_set)){
  
  herb_rate <- herb_set[j]
  
  C_L.simu.herb_rate <- NaN*tset.herb_rate; C_L.simu.herb_rate[1] <- 0.6
  C_D.simu.herb_rate <- NaN*tset.herb_rate; C_D.simu.herb_rate[1] <- 0
  M_S.simu.herb_rate <- NaN*tset.herb_rate; M_S.simu.herb_rate[1] <- 0.05
  M_C.simu.herb_rate <- NaN*tset.herb_rate; M_C.simu.herb_rate[1] <- 0.05
  ...
  for(i in 2:length(tset.herb_rate)){ # For every element after the first in the time vector...
    ...
    # Proportional cover at previous time step  
    C_L <- C_L.simu.herb_rate[i-1]
    C_D <- C_D.simu.herb_rate[i-1]
    M_S <- M_S.simu.herb_rate[i-1]
    M_C <- M_C.simu.herb_rate[i-1]
    ...
    # Proportional cover at new time step   
    C_L.simu.herb_rate[i] <- C_L.simu.herb_rate[i-1] + dC_L
    C_D.simu.herb_rate[i] <- C_D.simu.herb_rate[i-1] + dC_D
    M_S.simu.herb_rate[i] <- M_S.simu.herb_rate[i-1] + dM_S
    M_C.simu.herb_rate[i] <- M_C.simu.herb_rate[i-1] + dM_C
  }
  
  C_L.eq_cover.herb_rate[j] <- C_L.simu.herb_rate[max(tset.herb_rate)]
  C_D.eq_cover.herb_rate[j] <- C_D.simu.herb_rate[max(tset.herb_rate)]
  M_S.eq_cover.herb_rate[j] <- M_S.simu.herb_rate[max(tset.herb_rate)]
  M_C.eq_cover.herb_rate[j] <- M_C.simu.herb_rate[max(tset.herb_rate)]
}


# Plot of time series
plot(herb_set, C_L.eq_cover.herb_rate, col = "blue", lwd = 2, lty = 'solid', type = 'l', xlab = "Herbivory rate (h_m_c)", ylab = "Equilibrium cover", xlim = c(0, max(herb_set)), ylim = c(0, 1))
lines(herb_set, (M_S.eq_cover.herb_rate + M_C.eq_cover.herb_rate), col = "green", lwd = 2, lty = 'solid', type = 'l')
legend(x=0.2, y=0.6,legend=c('Coral','Total macroalgae'), col = c("blue", "green"), lty = c('solid', 'solid'))
