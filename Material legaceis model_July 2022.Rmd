---
title: "Material legacies can degrade resilience_Ecology revision"
author: "Kai Kopecky"
date: "7/28/2022"
...
Updated model code to include settlement of live coral onto dead coral skeleton.
...
\newline
&C_P\ =\ Live\ coral\ growing \ on \ dead \ coral \\
\newline
...
Rates of change for proportional spatial cover of live coral (either on on open substrate or dead coral), dead coral, and macroalgae (either open substrate or dead coral)
$$
\begin{align}
&\frac{d C_L}{dt} =  g_c C_L S + \gamma_cS - g_{mc} C_L(M_E + M_P) - d_c C_L \\ 
\newline
&\frac{d C_P}{dt} = g_c C_P C_D + s\gamma_cC_D - g_{mc} C_P (M_E + M_P) - d_c C_P - eC_P \\
\newline
&\frac{d C_D}{dt} = d_c (C_L + C_P) + M_P(h(1-p) + d_m) - g_c C_P C_D - \gamma_cC_D - g_m C_D (M_E + M_P) - \gamma_mC_D - e C_D \\ 
\newline
&\frac{d M_E}{dt} = g_m M_E S + g_m M_P S + \gamma_mS - M_E(h + d_m)\\
\newline
&\frac{d M_P}{dt} = g_m M_P C_D +  g_m M_E C_D + g_{mc} C_L(M_E + M_P) + g_{mc} C_P (M_E + M_P) + \gamma_mC_D - M_P(h(1-p) + d_m + e)\\
\newline
&\ S = 1 - C_L - C_P - C_D - M_E - M_P \\ 
\newline
\end{align}
$$
...
&g_{mc}\ =\ Growth\ rate\ of\ macroalgae\ onto\ live\ coral\\
&d_c\ =\ Ambient\ mortality\ of\ coral\\
&d_m\ =\ Ambient\ mortality\ of\ macroalgae\\
&e\ =\ Erosion\ of\ dead\ coral\\
&\gamma_c\ =\ Settlement\ rate\ of\ live\ coral\\
&s\ =\ Multiplier\ for\ rate\ of\ coral\ settlement\ onto\ dead\ coral\\ 
&\gamma_m\ =\ Settlement\ rate\ of\ macroalgae\\
&h\ =\ Ambient\ herbivory\ rate\\
&p\ =\ degree\ of\ protection\ from\ herbivory
&\end{align}
$$
...
gamma_C <- 0.001 # settlement of exogenously derived coral onto empty space or onto dead coral; range: 0.00006 - 0.01; default value within range
s <- 0 # multiplier for settlement of live coral onto dead coral that determines their survivorship rate relative to coral settlers on empty space
g_M <- 0.6 # growth of macroalgae onto open space or dead coral; range: 0.05 - 0.4
g_M_P_L <- 0.06 # growth of macroalgae over live coral; range: 0-0.9 of g_M; default value within range; notated as g_mc above
d_M <- 0.05 # mortality rate of macroalgae; no published range
h_M_E <- 0.3 # ambient herbivory rate; range: 0.05 - 5; default value within range
ref <- 0.5 # degree of refuge from herbivory for macroalgae that is provided by dead skeleton; no published range; notated as p above
...
Time series for structure-removing disturbance event in which all space-holders are reduced to 40% of their pre-disturbance cover. Starting conditions reflect a coral-dominated community state. 
```{r Figure 4 code, echo = FALSE}

# Create time vector, holding vectors for each state variable, and set initial conditions
C_L.simu_rem <- NaN*tset.disturb; C_L.simu_rem[1] <- 0.91 # Equilibrium value
C_P.simu_rem <- NaN*tset.disturb; C_P.simu_rem[1] <- 0
C_D.simu_rem <- NaN*tset.disturb; C_D.simu_rem[1] <- 0.08 # Equilibrium value
...
for(i in 2:length(tset.disturb)){ # For every element after the first in the time vector...
  ...
  dC_L <- (dt)*(g_C*C_L*(1-C_L-C_P-C_D-M_E-M_P) + gamma_C*(1-C_L-C_P-C_D-M_E-M_P) - g_M_P_L*C_L*(M_E + M_P) - d_C*C_L) 
  dC_P <- (dt)*(g_C*C_P*C_D + s*gamma_C*C_D - g_M_P_L*C_P*(M_E + M_P) - d_C*C_P - e*C_P)
  dC_D <- (dt)*(d_C*(C_L + C_P) + M_P*(h_M_E*(1-ref) + d_M) - g_C*C_P*C_D - s*gamma_C*C_D - g_M*C_D*(M_E + M_P) - gamma_M*C_D - e*C_D)
  dM_E <- (dt)*(g_M*M_E*(1-C_L-C_P-C_D-M_E-M_P) + g_M*M_P*(1-C_L-C_P-C_D-M_E-M_P) + gamma_M*(1-C_L-C_P-C_D-M_E-M_P) - M_E*(h_M_E + d_M))
  dM_P <- (dt)*(g_M*M_P*C_D + g_M*M_E*C_D + g_M_P_L*C_L*(M_E + M_P) + g_M_P_L*C_P*(M_E + M_P) + gamma_M*C_D - M_P*(h_M_E*(1-ref) + d_M + e))
 
  # Proportional cover at new time step   
  C_L.simu_rem[i] <- C_L.simu_rem[i-1] + dC_L
  C_P.simu_rem[i] <- C_P.simu_rem[i-1] + dC_P
  ...
  # reduce all space holders to 40% of their pre-disturbance levels
  C_L <- C_L.simu_rem[i-1]*0.4
  C_P <- C_P.simu_rem[i-1]*0.4
  C_D <- C_D.simu_rem[i-1]*0.4
  M_E <- M_E.simu_rem[i-1]*0.4
  M_P <- M_P.simu_rem[i-1]*0.4
...
lines(tset.disturb, C_P.simu_rem, col = kotare[2], lwd = 2, lty = 'dashed')
lines(tset.disturb, C_D.simu_rem, col = "black", lwd = 2, lty = 'solid')
lines(tset.disturb, M_E.simu_rem, col = kotare[3], lwd = 2, lty = 'solid') # Protected and exposed macroalgae combined to show cover of total macroalgae
lines(tset.disturb, M_P.simu_rem, col = kotare[3], lwd = 2, lty = 'dashed')
legend(.7*max(tset),.8,legend=c('Total Live Coral','Dead Coral','Total Macroalgae'),lwd=2,col=c(kotare[2],'black',kotare[3]))
abline(v=500, lty = 'dashed')

# Plot of time series (coral and algae classes grouped)
plot(tset.disturb, (C_L.simu_rem + C_P.simu_rem), col = kotare[2], lwd = 2, lty = 'solid', type = 'l', xlab = "Time (years)", ylab = "Proportional cover", xlim = c(0, max(tset.disturb)), ylim = c(0, 1))#, main = "Structure-removing")
lines(tset.disturb, C_D.simu_rem, col = "black", lwd = 2, lty = 'solid')
...
legend(.6*max(tset.disturb),.8,legend=c('Total Live Coral','Dead Coral','Total Macroalgae'),lwd=2,col=c(kotare[2],'black',kotare[3]))
...
# Create time vector, holding vectors for each state variable, and set initial conditions
C_L.simu_ret <- NaN*tset.disturb; C_L.simu_ret[1] <- 0.91
C_P.simu_ret <- NaN*tset.disturb; C_P.simu_ret[1] <- 0
C_D.simu_ret <- NaN*tset.disturb; C_D.simu_ret[1] <- 0.08
...
# Proportional cover at new time step  
  ...
  C_P.simu_ret[i] <- C_P.simu_ret[i-1] + dC_P
  ...
  C_L <- C_L.simu_ret[i-1]*0.4 # reduce live coral (open substrate) to 40% of its pre-disturbance level...
  C_P <- C_P.simu_ret[i-1]*0.4 # and reduce live coral (dead coral) to 40% of its pre-disturbance level...
  C_D <- C_D.simu_ret[i-1] + (C_L.simu_ret[i-1]-C_L) + (C_P.simu_ret[i-1]-C_P) #... then transfer these directly to the dead coral class
  M_E <- M_E.simu_ret[i-1]
  M_P <- M_P.simu_ret[i-1]
...
lines(tset.disturb, C_P.simu_ret, col = kotare[2], lwd = 2, lty = 'dashed')
lines(tset.disturb, C_D.simu_ret, col = "black", lwd = 2, lty = 'solid')
lines(tset.disturb, M_E.simu_ret, col = kotare[3], lwd = 2, lty = 'solid') # Protected and exposed macroalgae combined to show cover of total macroalgae
lines(tset.disturb, M_P.simu_ret, col = kotare[3], lwd = 2, lty = 'dashed')
abline(v=500, lty = 'dashed')

# Plot of time series for all space holders (coral and algae classes grouped)
plot(tset.disturb, (C_L.simu_ret + C_P.simu_ret), col = kotare[2], lwd = 2, lty = 'solid', type = 'l', xlab = "Time (years)", ylab = "Proportional cover", xlim = c(0, max(tset.disturb)), ylim = c(0, 1))#, main = "Structure-retaining")
lines(tset.disturb, C_D.simu_ret, col = "black", lwd = 2, lty = 'solid')
...
## Creating a funciton for the model

Here, we create a function representing the model so that we don't have to paste all that code in each time.

```{r}
CoralFun <- function(tset,ic,t_disturb,dist.type,disturbance_severity,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M){
  # create holding vectors for the inner loop
	...
	C_P <- NaN*tset; 	C_P[1] <- ic[2]
	C_D <- NaN*tset; 	C_D[1] <- ic[3]
	M_E <- NaN*tset;	M_E[1] <- ic[4]
	M_P <- NaN*tset; 	M_P[1] <- ic[5]
  ...
  if(tset[i] !=t_disturb){ # if this is not a disturbance event
    ...
    dC_P <- (dt)*(g_C*C_P[i-1]*C_D[i-1] + s*gamma_C*C_D[i-1] - g_M_P_L*C_P[i-1]*(M_E[i-1] + M_P[i-1]) - d_C*C_P[i-1] - e*C_P[i-1])
    dC_D <- dt*(d_C*C_L[i-1] + M_P[i-1]*(h_M_E*(1-ref) + d_M) - g_C*C_P[i-1]*C_D[i-1] - s*gamma_C*C_D[i-1] - g_M*C_D[i-1]*(M_E[i-1] + M_P[i-1]) - gamma_M*C_D[i-1] - e*C_D[i-1])
    ...
    dM_P <- dt*(g_M*M_P[i-1]*C_D[i-1] + g_M*M_E[i-1]*C_D[i-1] + g_M_P_L*C_L[i-1]*(M_E[i-1] + M_P[i-1]) + g_M_P_L*C_P[i-1]*(M_E[i-1] + M_P[i-1]) + gamma_M*C_D[i-1] - M_P[i-1]*(h_M_E*(1-ref) + d_M + e)) 
  	...
  	M_P[i] <- M_P[i-1] + dM_P
        ...
        if(dist.type == 'ret'){
          ...
          C_P[i] <- C_P[i-1]*(1-disturbance_severity)
          C_D[i] <- C_D[i-1] + (C_L[i-1] - C_L[i]) + (C_P[i-1] - C_P[i])
          M_E[i] <- M_E[i-1]
          M_P[i] <- M_P[i-1]
          ...
          C_L[i] <- C_L[i-1]*(1-disturbance_severity)
          C_P[i] <- C_L[i-1]*(1-disturbance_severity)
	...
	Results <- list("CL" = C_L, "CP" = C_P, "CD" = C_D, "ME" = M_E, "MP" = M_P) # concatenate results into a list
...
Res <- CoralFun(tset,c(1,0,0,0,0),t_disturb,'none',disturbance_severity,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)

plot(tset,Res$CL,las=1,xlab='Time',ylab='Prop. Cover',type='l',col='coral',lwd=2,ylim=c(0,1))
lines(tset,Res$CP,col='grey',lwd=2)
lines(tset,Res$CD,col='black',lwd=2)
lines(tset,Res$ME,col='green',lwd=2)
lines(tset,Res$MP,col='darkgreen',lwd=2)
legend(.7*max(tset),.8,legend=c('Live Coral (open)','Live coral (dead coral)','Dead Coral','Exp. Mac.','Prot. Mac.'),lwd=2,col=c('coral','grey','black','green','darkgreen'))

ic.cor <- c(Res$CL[length(tset)],Res$CP[length(tset)],Res$CD[length(tset)],Res$ME[length(tset)],Res$MP[length(tset)])


# Find low-coral conditions:
Res <- CoralFun(tset,c(0,0,0,.5,.5),t_disturb,'none',disturbance_severity,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
...
```{r}
CoralFun.NoDisturb <- function(tset,ic,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M){
  # create holding vectors for the inner loop
	C_L <- NaN*tset; 	C_L[1] <- ic[1]# use the vector of initial conditions to populate initial densities
	C_P <- NaN*tset; 	C_P[1] <- ic[2]
	C_D <- NaN*tset; 	C_D[1] <- ic[3]
...
DistSevThresh.adapt <- function(ic.use,dist.type,tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M,iters){
  ...
  if(dist.type=='ret'){ic.dist <- c(ic.use[1]*(1-dist_low),ic.use[2]*(1-dist_low),ic.use[3]+ic.use[1]*dist_low + ic.use[2]*dist_low,ic.use[4],ic.use[5])} #c(ic.use[1]*(1-dist_low),ic.use[2]+ic.use[1]*dist_low,ic.use[3],ic.use[4])}
  if(dist.type=='rem'){ic.dist <- ic.use*(1-dist_low)}
  # Run the simulation
  Res <- CoralFun.NoDisturb(tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  # Save the results
  Cstar_low <- Res$CL[length(tset)] + Res$CP[length(tset)]
  
  ## Step 1B: Check upper bound (disturbance = 1)
  dist_high <- 1
  # Adjust initial conditions based on the disturbance severity and type
  if(dist.type=='ret'){ic.dist <- c(ic.use[1]*(1-dist_high),ic.use[2]*(1-dist_high),ic.use[3]+ic.use[1]*dist_high + ic.use[2]*dist_high,ic.use[4],ic.use[5])} #c(ic.use[1]*(1-dist_high),ic.use[2]+ic.use[1]*dist_high,ic.use[3],ic.use[4])}
  if(dist.type=='rem'){ic.dist <- ic.use*(1-dist_high)}
  # Run the simulation
  Res <- CoralFun.NoDisturb(tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  # Save the results
  Cstar_high <- Res$CL[length(tset)]+ Res$CP[length(tset)]
  ...
  # Adjust initial conditions based on the disturbance severity and type
  if(dist.type=='ret'){ic.dist <- c(ic.use[1]*(1-dist_test),ic.use[2]*(1-dist_test),ic.use[3]+ic.use[1]*dist_test + ic.use[2]*dist_test,ic.use[4],ic.use[5])}#c(ic.use[1]*(1-dist_test),ic.use[2]+ic.use[1]*dist_test,ic.use[3],ic.use[4])}
  if(dist.type=='rem'){ic.dist <- ic.use*(1-dist_test)}
  # Run the simulation
  Res <- CoralFun.NoDisturb(tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  # Save the results
  Cstar_test <- Res$CL[length(tset)]+ Res$CP[length(tset)]
...
```{r}
  ...
  Res <- CoralFun.NoDisturb(tset,c(1,0,0,0,0),g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  ic.cor <- c(Res$CL[length(tset)],Res$CP[length(tset)],Res$CD[length(tset)],Res$ME[length(tset)],Res$MP[length(tset)])
...
for(k in 1:length(iters.set)){
  iters <- iters.set[k]
  Res2 <- DistSevThresh.adapt(ic.use,dist.type,tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M,iters)
...
for(ctr2 in 1:length(ref_set)){
  ...
  Res <- CoralFun.NoDisturb(tset,c(1,0,0,0,0),g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  ic.cor <- c(Res$CL[length(tset)],Res$CP[length(tset)],Res$CD[length(tset)],Res$ME[length(tset)],Res$MP[length(tset)])
  C.eq.set[ctr2] <- Res$CL[length(tset)] + Res$CP[length(tset)] # Record the equilibrium coral abundance in the coral-dominated state
  
  # Find low-coral conditions:
  Res <- CoralFun.NoDisturb(tset,c(0,0,0,0.5,0.5),g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  ic.mac <- c(Res$CL[length(tset)],Res$CP[length(tset)],Res$CD[length(tset)],Res$ME[length(tset)],Res$MP[length(tset)])
  M.eq.set[ctr2] <- Res$CL[length(tset)] + Res$CP[length(tset)] # Record the equilibrium coral abundance in the macroalgal-dominated state
...
ReturnTime <- function(ic.cor,ic.mac,tset,ic,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M){
  # Create holding vectors for the inner loop
	C_L <- NaN*tset; 	C_L[1] <- ic[1] # use the vector of initial conditions to populate initial densities
	C_P <- NaN*tset; 	C_P[1] <- ic[2]
	C_D <- NaN*tset; 	C_D[1] <- ic[3]
...
# 	if(C_L[length(tset)] < .5){Eq = 2} # Modification: If coral abundance is below 0.5, then assume it's macroalgal dominated. CAUTION!!! Check this carefully because if you choose different parameters, this threshold might be different.
  ...
  if (abs((C_L[length(tset)]-ic.cor[1]) + (C_P[length(tset)]-ic.cor[2])) < 0.001) {EqCorUse <- ic.cor[1]; EqMacUse <- ic.cor[4]; EqCPUse <- ic.cor[2]; EqCDUse <- ic.cor[3]; EqMCUse <- ic.cor[5]; Eq = 1} else {EqCorUse <- ic.mac[1]; EqMacUse <- ic.mac[4]; EqCPUse <- ic.mac[2]; EqCDUse <- ic.mac[3]; EqMCUse <- ic.mac[5]; Eq = 2}
	if(C_L[length(tset)] + C_P[length(tset)] < .5){Eq = 2} # Modification: If coral abundance is below 0.5, then assume it's macroalgal dominated. CAUTION!!! Check this carefully because if you choose different parameters, this threshold might be different.

  # Figure out how long it took to get there
  RetTime <- tset[which(tset*(abs(C_L-EqCorUse) + abs(M_E-EqMacUse) + abs(C_P-EqCPUse) + abs(C_D-EqCDUse) + abs(M_P-EqMCUse) < 0.005) != 0)[1]] # Which is the first timestep that you're near the equilibrium?
  
  Results <- list("RetTime" = RetTime,"CL" = C_L, "CP" = C_P, "CD" = C_D, "ME" = M_E, "MP" = M_P,"Eq" = Eq)
  Results
}
...
for(varctr in 1:length(ref_set)){
  ...
  Res <- CoralFun.NoDisturb(tset,c(1,0,0,0,0),g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  ic.cor <- c(Res$CL[length(tset)],Res$CP[length(tset)],Res$CD[length(tset)],Res$ME[length(tset)],Res$MP[length(tset)])
  C.eq.set[varctr] <- Res$CL[length(tset)] + Res$CP[length(tset)] # Record the equilibrium coral abundance in the coral-dominated state
  
  # Find low-coral conditions:
  Res <- CoralFun.NoDisturb(tset,c(0,0,0,0.5,0.5),g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
  ic.mac <- c(Res$CL[length(tset)],Res$CP[length(tset)],Res$CD[length(tset)],Res$ME[length(tset)],Res$MP[length(tset)])
  M.eq.set[varctr] <- Res$CL[length(tset)] + Res$CP[length(tset)]# Record the equilibrium coral abundance in the macroalgal-dominated state
  ...
  if(abs(C.eq.set[varctr] -M.eq.set[varctr]) > 0.001){ # Only bother finding tipping points if there are two stable equilibria (e.g., the coral cover in coral and macroalgal dominated systems is different)
    ...
    ic.dist <- c(ic.cor[1]*(1-dist.use),ic.cor[2]*(1-dist.use),ic.cor[3]+ic.cor[1]*dist.use+ic.cor[2]*dist.use,ic.cor[4],ic.cor[5]) # start with a coral dominated system 
    Res2 <- ReturnTime(ic.cor,ic.mac,tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
    ...
    ic.dist <- c(ic.mac[1]*(1-dist.use),ic.mac[2]*(1-dist.use),ic.mac[3]+ic.mac[1]*dist.use+ic.mac[2]*dist.use,ic.mac[4],ic.mac[5]) # start with a macroalgal dominated system
    Res2 <- ReturnTime(ic.cor,ic.mac,tset,ic.dist,g_C,gamma_C,s,g_M_P_L,d_C,h_M_E,ref,d_M,g_M,e,gamma_M)
...
image.plot(x = ref_set, y = dist.prop.set, z = C.rem.time.set.toC ,las=1,col=coral.col(100),xlab= "Algal Protection, p",ylab="Disturbance Intensity",xlim=c(0.43,0.78), ylim=c(0,1), zlim=zlimits)#, main = 'Structure-removing, initial state = coral')
image.plot(x = ref_set, y = dist.prop.set, z = C.rem.time.set.toM ,las=1,col=macroalgae.col(100),add=T,zlim=zlimits,xlim=c(0.43,0.78), ylim=c(0,1))
CtoM.rem.set2 <- CtoM.rem.set
CtoM.rem.set2[CtoM.rem.set2==0] <- 1
lines(ref_set, CtoM.rem.set2, lwd=3, col = "black")
...
image.plot(x = ref_set, y = dist.prop.set, z = C.ret.time.set.toC ,las=1,col=coral.col(100),xlab='Algal protection, p',ylab='Disturbance Intensity',xlim=c(0.43,0.78), ylim=c(0,1), zlim=zlimits)#, main = 'Structure-retaining, initial state = coral')
image.plot(x = ref_set, y = dist.prop.set, z = C.ret.time.set.toM ,las=1,col=macroalgae.col(100),add=T,zlim=zlimits,xlim=c(0.43,0.78),ylim=c(0,1))
lines(ref_set, CtoM.ret.set, lwd=3, col = 'black')
...
image.plot(x = ref_set, y = dist.prop.set, z = M.rem.time.set.toC ,las=1,col=coral.col(100),xlab='Algal Protection, p',ylab='Disturbance Intensity',xlim=c(0.43,0.78), ylim=c(0,1), zlim=zlimits)#, main = 'Structure-removing, initial state = macroalgae')
image.plot(x = ref_set, y = dist.prop.set, z = M.rem.time.set.toM ,las=1,col=macroalgae.col(100),add=T,zlim=zlimits,xlim=c(0.43,0.78),ylim=c(0,100))
lines(ref_set, MtoC.rem.set, lwd=3, col = 'black')
...
image.plot(x = ref_set, y = dist.prop.set, z = M.ret.time.set.toC ,las=1,col=coral.col(100),xlab='Algal Protection, p',ylab='Disturbance Intensity',xlim=c(0.43,0.78), ylim=c(0,1), zlim=zlimits)#, main = 'Structure-retaining, initial state = macroalgae')
image.plot(x = ref_set, y = dist.prop.set, z = M.ret.time.set.toM ,las=1,col=macroalgae.col(100),add=T,zlim=zlimits,xlim=c(0.43,0.78),ylim=c(0,1))
